...
export function AffordabilityAnalyzer() {
  const [result, setResult] = useState<AffordabilityAnalysisOutput | null>(
    null
  );
    ...
    if (purchaseCost && downPayment && interestRate && loanTerm) {
      ...
      if (annualIncome && monthlyExpenses) {
        ...
        const affordability = Math.max(
          0,
          Math.min(100, ((availableIncome - emi) / availableIncome) * 100)
        );
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const { analyzeAffordability } = await import(
        "@/ai/flows/affordability-analysis"
      );
      const response = await analyzeAffordability(values);
      setResult(response);

      // Fetch RAG insights after affordability analysis
  ...
  const getRagInsights = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const queryText = `Purchase affordability analysis: ${
        values.purchaseDescription
      } costing ₹${values.purchaseCost.toLocaleString()}. 
                     Monthly income: ₹${monthlyIncome.toLocaleString()}, Monthly expenses: ₹${values.monthlyExpenses.toLocaleString()}, 
                     EMI: ₹${monthlyEMI.toLocaleString()}, Affordability score: ${affordabilityScore.toFixed(
        1
      )}%. 
                     What do similar income profiles typically spend on ${
                       values.purchaseDescription
                     }? 
                     What are the recommended budget ranges and financing strategies from the dataset?`;
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p className="max-w-xs">
                            Our AI analyzes your financial data to provide
                            personalized affordability insights and
                            recommendations.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    ...
                    {affordabilityScore > 0 && (
                      <div className="flex items-center space-x-3">
                        ...
                        >
                          {(() => {
                            const IconComponent =
                              getAffordabilityIcon(affordabilityScore);
                            return (
                              <IconComponent
                                className={`h-4 w-4 ${getAffordabilityColor(
                                  affordabilityScore
                                )}`}
                              />
                            );
                        ...
                        <FormItem>
                          <FormLabel className="text-sm font-semibold flex items-center space-x-2">
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <p>
                                  Initial payment made when purchasing. Higher
                        ...
                        <FormItem>
                          <FormLabel className="text-sm font-semibold flex items-center space-x-2">
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <p>
                                  Include rent, utilities, food, transportation,
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <p>
                                  ...
                                  Current rates: Car 8-12%, Home 8-10%.
                                </p>
                              </TooltipContent>
                            </Tooltip>
                          </FormLabel>
              ...
              <Card className="w-full max-w-5xl mx-auto shadow-lg">
                <CardHeader>
                  <CardTitle className="font-headline flex items-center gap-2">
                    ...
                    Spending Benchmarks & Market Insights
                  </CardTitle>
                  <CardDescription>
                    Comparing your purchase with 15,000 similar financial
                    profiles
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {isLoadingRag ? (
                    <div className="p-6 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg">
                      <div className="flex items-center gap-2">
                        <Sparkles className="w-5 h-5 text-emerald-600 animate-pulse" />
                        <p className="text-sm text-muted-foreground">
                          Analyzing purchase patterns and affordability
                          benchmarks...
                        </p>
                      </div>
                    </div>
                  ) : ragInsights ? (
                    <div className="p-6 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg space-y-3">
